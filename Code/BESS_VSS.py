...
def EV(config=cfg.currentConfig):
    ...
    # Expected value of prices
    pSpot_Exp = np.tile(np.average(pSpot, axis=0, weights=prob), (cfg.scenarioCount, 1))
    pLU_Exp = np.tile(np.average(pLU, axis=0, weights=prob), (cfg.scenarioCount, 1))
    pLD_Exp = np.tile(np.average(pLU, axis=0, weights=prob), (cfg.scenarioCount, 1))
    ...
    m.setObjective(expr=(
            ...
            + gp.quicksum(prob[s] * (pSpot_Exp[s, h] * qSpot[s, h]) for s in range(cfg.scenarioCount) for h in range(cfg.hourCount))
            + gp.quicksum(prob[s] * (pLU_Exp[s, h] * qLU[s, h]) for s in range(cfg.scenarioCount) for h in range(cfg.hourCount))
            + gp.quicksum(prob[s] * (pLD_Exp[s, h] * qLD[s, h]) for s in range(cfg.scenarioCount) for h in range(cfg.hourCount))
    ),
        sense=GRB.MAXIMIZE
    )
    ...
    for h in range(cfg.hourCount):
        dataEU[h] = [round(m.getVarByName(f"{var}[{s},{h}]").x, 2) for var in ["qEU"] for s in range(cfg.scenarioCount)]
        dataED[h] = [round(m.getVarByName(f"{var}[{s},{h}]").x, 2) for var in ["qED"] for s in range(cfg.scenarioCount)]
    dfDataEU = pd.DataFrame(dataEU)
    dfDataED = pd.DataFrame(dataED)
    dfDataEU.to_csv(f"{pathConfig}DataEU.csv", index=False)
    dfDataED.to_csv(f"{pathConfig}DataED.csv", index=False)



def EEV(config=cfg.currentConfig):
    logger.info(f"Running configuration '{config}'")
    pathConfig = f"{cfg.pathData}{config}/"
    # For documentation, see
    # https://www.gurobi.com/documentation/11.0/refman/py_model.html
    ...
    # Import data from Expected Value Problem
    dfDataEU = pd.read_csv(f"{pathConfig}DataEU.csv")
    dfDataED = pd.read_csv(f"{pathConfig}DataED.csv")
    npDataEU = dfDataEU.to_numpy()
    npDataED = dfDataED.to_numpy()
    ...
    # FROM EXPECTED VALUE PROBLEM
    m.addConstrs(qEU[s, h] == npDataEU[s, h] for h in range(cfg.hourCount) for s in range(cfg.scenarioCount))
    m.addConstrs(qED[s, h] == npDataED[s, h] for h in range(cfg.hourCount) for s in range(cfg.scenarioCount))
    ...
    if m.Status == 2:
        ...
        logger.warning(f"Check status code {m.Status}")
        
    logger.info(f"Exporting optimal variables")
    data = {}
    varList = ["qEU", "qED", "qSpot", "qLU", "qLD"]
    ...
    dfdata.to_csv(f"{cfg.pathCurrentConfig}DataEEV.csv", index=False)
...
def runVSS(config=cfg.currentConfig):
    startEEV = dt.datetime.now()
    logger.info("Running EV")
    EV(config)
    logger.info("Finished EV")
    logger.info("------------------------------------")
    logger.info("Running EEV")
    objV, profitEU, profitED, profitSpot, profitLU, profitLD = EEV(config)
    logger.info("Finished EEV")
    endEEV = dt.datetime.now()
    logger.info("------------------------------------")
    logger.info(f"Runtime {(endEEV-startEEV).total_seconds()}")
    logger.info("------------------------------------")
    ...
    data[config]["EEV"] = round(objV, 2)
    data[config]["EU_EEV"] = round(profitEU, 2)
    data[config]["ED_EEV"] = round(profitED, 2)
    data[config]["Spot_EEV"] = round(profitSpot, 2)
    data[config]["LU_EEV"] = round(profitLU, 2)
    data[config]["LD_EEV"] = round(profitLD, 2)
    data[config]["RuntimeEEV"] = round((endEEV-startEEV).total_seconds(),2)
