...
fn.checkCreateDir(cfg.pathCurrentConfig)
...
logger.info(f"Creating cluster for configuration '{cfg.currentConfig}'")
...
if cfg.stg3clusters > 1:
    kmedoids_stg3 = KMedoids(n_clusters=cfg.stg3clusters, init=cfg.init, random_state=cfg.random_state, metric='precomputed', max_iter=cfg.max_iter, method='pam').fit(npDist_stg3)
else:
    kmedoids_stg3 = KMedoids(n_clusters=cfg.stg3clusters, init=cfg.init, random_state=cfg.random_state, metric='precomputed', max_iter=0, method='pam').fit(npDist_stg3)
...
if cfg.stg2clusters > 1:
    kmedoids_stg2 = KMedoids(n_clusters=cfg.stg2clusters, init=cfg.init, random_state=cfg.random_state, metric='precomputed', max_iter=cfg.max_iter, method='pam').fit(npDist_stg2)
else:
    kmedoids_stg2 = KMedoids(n_clusters=cfg.stg2clusters, init=cfg.init, random_state=cfg.random_state, metric='precomputed', max_iter=0, method='pam').fit(npDist_stg2)
...
kmedoids_stg1 = KMedoids(n_clusters=1, init=cfg.init, random_state=cfg.random_state, metric='precomputed', max_iter=0, method='pam').fit(npDist_stg1)
...
# Export to CSV
logger.info(f"Exporting to .csv files")
dfPricesEarly.to_csv(f"{cfg.pathCurrentConfig}PricesEarly.csv", index=False)
dfPricesSpot.to_csv(f"{cfg.pathCurrentConfig}PricesSpot.csv", index=False)
dfPricesLate.to_csv(f"{cfg.pathCurrentConfig}PricesLate.csv", index=False)
dfProbabilities.to_csv(f"{cfg.pathCurrentConfig}Probabilities.csv", index=False)
dfGroup.to_csv(f"{cfg.pathCurrentConfig}Group.csv", index=False)
...
if cfg.currentConfig not in data:
    data[cfg.currentConfig] = {}

for item in ["EU", "ED", "Spot", "LU", "LD", "Objective", "EEV", "Runtime"]:
    if item not in data[cfg.currentConfig]:
        data[cfg.currentConfig][item]=0



data[cfg.currentConfig]["RuntimeClustering"] = (end-start).total_seconds()
...
dfMean = pd.DataFrame([np.average(np.average(price, axis=0, weights=prob)) for price in pList], columns=[cfg.currentConfig])
